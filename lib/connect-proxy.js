...
function ConnectProxy(options) {
  ...
  self.server = http.createServer(function (req, res) {
    ...
    if (proxy && whitelist.isWhite(dest.hostname)) {
      ...
      target = { host: dest.hostname, port: dest.port || 80 };
    }
  ...
  self.server.on('connect', function (req, socket, head) {
    ...
    if (proxy && whitelist.isWhite(dest.hostname)) {
      ...
      target = { host: dest.hostname, port: dest.port || 443 };
      isWhite = false;
    }

    var proxySocket = net.createConnection(target.port, target.host, function () {
      if (isWhite) {
